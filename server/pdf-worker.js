...
const worker = new Worker(
  ...
  async (job) => {
    try {
      ...
      await fs.access(path); // Ensure file exists

      const loader = new PDFLoader(path);
      const docs = await loader.load();

      if (docs.length === 0) throw new Error('PDF empty');
      ...
      try { await qClient.getCollection(collectionName); } catch { 
        await qClient.createCollection(collectionName, { vectors: { size: 768, distance: 'Cosine' } }); 
      }
      ...
      await fs.unlink(path); // optional cleanup
